...
with st.sidebar:
    ...
    if model_loaded:
        ...
        with st.expander("🔧 Debug info"):
            ...
            st.write(f"**Scaler expects:** {n} features")
            st.write(f"**App sends:** {len(TRAINING_COLUMNS)} features")
            if n != len(TRAINING_COLUMNS):
                st.error(
                    f"Mismatch! Edit TRAINING_COLUMNS in app.py "
                    f"so it has exactly {n} entries matching your training data."
                )
            else:
...
def encode_inputs(
    ...
    """
    Build a single-row DataFrame whose columns match TRAINING_COLUMNS exactly.

    Strategy:
      1. Put the raw categorical values into a tiny DataFrame.
      2. Use pd.get_dummies() to one-hot encode them — this produces
         the same column names that were generated during training.
      3. Reindex to TRAINING_COLUMNS so every expected column is present
         (filling any missing dummy columns with 0).

    This guarantees the feature matrix fed to scaler.transform() always
    has the right shape and column order, regardless of which option the
    user selected.
    """
    # ── Step 1: raw row ──
    row = pd.DataFrame([{
    ...
    # ── Step 2: one-hot encode the string columns ──
    row_encoded = pd.get_dummies(
        row,
        columns=["Partner", "Dependents", "Contract", "InternetService", "PaymentMethod"],
    )

    # ── Step 3: reindex to training schema (fills missing cols with 0) ──
    row_final = row_encoded.reindex(columns=TRAINING_COLUMNS, fill_value=0)

    return row_final.values  # shape (1, len(TRAINING_COLUMNS))
